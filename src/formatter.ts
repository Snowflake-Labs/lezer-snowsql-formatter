...
export function format(input: string) {
  ...
  function parse(
    d: string,
    dialectOpt: string,
    fragments?: readonly TreeFragment[] | any
  ) {
  ...
  function keywordFormatterNoNewline(
    cursor: TreeCursor,
    indent: number
  ): string {
    return basicIndent(indent) + sliceDoc(cursor) + " ";
  ...
  function selectDefinitionFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output: string = "";
  ...
  function fromExpressionFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output: string = "";
  ...
  function baseFromExpressionFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output: string = "";
  ...
  function joinExpressionFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output: string = "";
    ...
    if (tempCursor.firstChild()) {
      do {
        if (
          tempCursor.type.name == "CrossAndNaturalJoinKW" ||
          tempCursor.type.name == "JoinKW"
        ) {
          output += "\n" + targetFormatter(tempCursor, " ");
        }
        if (
          tempCursor.type.name == "DBTDlcly" ||
  ...
  function selectTargetListFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output: string = "";
  ...
  function scalarExpressionFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output: string = "";
  ...
  function createIntegrationFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output: string = "";
  ...
  function IntegrationTypesFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output = "";
    ...
    if (tempCursor.firstChild()) {
      do {
        if (
          ["Api", "Storage", "Security", "Notification"].includes(
            tempCursor.type.name
          )
        ) {
  ...
  function KeyValuePropertyFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output = "";
  ...
  function noSpacedSymbolFormatter(
    cursor: TreeCursor,
    indent?: number
  ): string {
    var output: string = "";
        ...
        if (enter) {
          ...
          if (!isTop || node.from > 0) {
            ...
            const hasNextSibling =
              leafCursor.nextSibling() && leafCursor.prevSibling();
            if (hasNextSibling) {
              output += " ├─ ";
              prefixes.push(" │  ");
            } else {
